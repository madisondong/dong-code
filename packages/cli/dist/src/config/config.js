...
import { Config, loadServerHierarchicalMemory, setGeminiMdFilename as setServerGeminiMdFilename, getCurrentGeminiMdFilename, ApprovalMode, DEFAULT_GEMINI_MODEL, DEFAULT_GEMINI_EMBEDDING_MODEL, DEFAULT_MEMORY_FILE_FILTERING_OPTIONS, FileDiscoveryService, IdeClient, } from '@dong-code/dong-code-core';
...
export async function parseArguments() {
    const yargsInstance = yargs(hideBin(process.argv))
        .scriptName('dong')
        .usage('$0 [options]', 'Dong Code - Launch an interactive CLI, use -p/--prompt for non-interactive mode')
        .option('model', {
        alias: 'm',
        type: 'string',
        description: `Model`,
...
export async function loadHierarchicalGeminiMemory(currentWorkingDirectory, debugMode, fileService, settings, extensionContextFilePaths = [], memoryImportFormat = 'tree', fileFilteringOptions) {
    ...
    return loadServerHierarchicalMemory(effectiveCwd, debugMode, fileService, extensionContextFilePaths, memoryImportFormat, fileFilteringOptions, settings.memoryDiscoveryMaxDirs);
}
export async function loadCliConfig(settings, extensions, sessionId, argv) {
    ...
    const { memoryContent, fileCount } = await loadHierarchicalGeminiMemory(process.cwd(), debugMode, fileService, settings, extensionContextFilePaths, memoryImportFormat, fileFiltering);
