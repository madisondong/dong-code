...
import { getErrorMessage, getAllGeminiMdFilenames, ApprovalMode, isEditorAvailable, FlashFallbackEvent, logFlashFallback, AuthType, ideContext, } from '@dong-code/dong-code-core';
...
import { isProQuotaExceededError, isGenericQuotaExceededError, UserTierId, } from '@dong-code/dong-code-core';
...
const App = ({ config, settings, startupWarnings = [], version }) => {
    ...
    const performMemoryRefresh = useCallback(async () => {
        addItem({
            ...
            text: 'Refreshing hierarchical memory (DONG.md or other context files)...',
        }, Date.now());
        try {
...
⚡ You can switch authentication methods by typing /auth`;
    ...
    const { handleSlashCommand, slashCommands, pendingHistoryItems: pendingSlashCommandHistoryItems, commandContext, shellConfirmationRequest, } = useSlashCommandProcessor(config, settings, addItem, clearItems, loadHistory, refreshStatic, setDebugMessage, openThemeDialog, openAuthDialog, openEditorDialog, toggleCorgiMode, setQuittingMessages, openPrivacyNotice, toggleVimEnabled, setIsProcessing);
    const { streamingState, submitQuery, initError, pendingHistoryItems: pendingGeminiHistoryItems, thought, } = useGeminiStream(config.getGeminiClient(), history, addItem, config, setDebugMessage, handleSlashCommand, shellModeActive, getPreferredEditor, onAuthError, performMemoryRefresh, modelSwitchedFromQuotaError, setModelSwitchedFromQuotaError);
    // Input handling
