...
import { DiscoveredMCPTool, getMCPDiscoveryState, getMCPServerStatus, MCPDiscoveryState, MCPServerStatus, mcpServerRequiresOAuth, getErrorMessage, } from '@dong-code/dong-code-core';
...
const getMcpStatus = async (context, showDescriptions, showSchema, showTips = false) => {
    ...
    for (const serverName of serverNames) {
        ...
        if (server?.oauth?.enabled) {
            needsAuthHint = true;
            try {
                const { MCPOAuthTokenStorage } = await import('@dong-code/dong-code-core');
...
const authCommand = {
    ...
    action: async (context, args) => {
        ...
        try {
            ...
            const { MCPOAuthProvider } = await import('@dong-code/dong-code-core');
