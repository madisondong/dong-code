...
import * as ServerConfig from '@dong-code/dong-code-core';
...
vi.mock('@dong-code/dong-code-core', async () => {
  const actualServer = await vi.importActual<typeof ServerConfig>(
    '@dong-code/dong-code-core',
  );
  return {
    ...actualServer,
    IdeClient: {
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
    ...
    const extensions: Extension[] = [
      {
        ...
        contextFiles: ['/path/to/ext1/DONG.md'],
      },
      {
        config: {
          name: 'ext2',
  ...
  it('should correctly use mocked homedir for global path', async () => {
    ...
    const MOCK_GLOBAL_PATH_LOCAL = path.join(MOCK_GEMINI_DIR_LOCAL, 'DONG.md');
    mockFs({
