...
describe('OpenAIContentGenerator Timeout Handling', () => {
    ...
    describe('generateContentStream timeout handling', () => {
        it('should handle streaming timeout errors', async () => {
            ...
            await expect(generator.generateContentStream(request, 'test-prompt-id')).rejects.toThrow(/Streaming setup timeout after \d+s\. Try reducing input length or increasing timeout in config\./);
        });
        it('should include streaming-specific troubleshooting tips', async () => {
            ...
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : String(error);
                expect(errorMessage).toContain('Streaming setup timeout troubleshooting:');
                expect(errorMessage).toContain('Check network connectivity and firewall settings');
