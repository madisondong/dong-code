...
export class OpenAIContentGenerator {
    ...
    async generateContent(request, userPromptId) {
        ...
        try {
            ...
            const errorEvent = new ApiResponseEvent(this.model, durationMs, userPromptId, this.config.getContentGeneratorConfig()?.authType, estimatedUsage, undefined, errorMessage);
        ...
        try {
            ...
            const wrappedGenerator = async function* () {
                try {
                    ...
                    // Log API response event for UI telemetry
                    const responseEvent = new ApiResponseEvent(this.model, durationMs, userPromptId, this.config.getContentGeneratorConfig()?.authType, finalUsageMetadata);
                    logApiResponse(this.config, responseEvent);
     ...
     */
    async convertGeminiRequestToOpenAI(request) {
        const messages = [];
        ...
        const mergedMessages = this.mergeConsecutiveAssistantMessagesForLogging(cleanedMessages);
        const openaiRequest = {
            model: this.model,
     ...
     */
    cleanOrphanedToolCallsForLogging(messages) {
        const cleaned = [];
        const toolCallIds = new Set();
        const toolResponseIds = new Set();
        // First pass: collect all tool call IDs and tool response IDs
     ...
     */
    mergeConsecutiveAssistantMessagesForLogging(messages) {
        const merged = [];
        for (const message of messages) {
            if (message.role === 'assistant' && merged.length > 0) {
                const lastMessage = merged[merged.length - 1];
