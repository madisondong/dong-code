...
describe('QwenOAuth2Client', () => {
    ...
    describe('refreshAccessToken', () => {
        ...
        it('should cache credentials after successful refresh', async () => {
            ...
            const cachedCredentials = JSON.parse(writeCall[1]);
    ...
    describe('getAccessToken', () => {
        ...
        it('should refresh token if access token is expired', async () => {
            ...
            vi.mocked(global.fetch).mockResolvedValue(mockRefreshResponse);
            const result = await client.getAccessToken();
...
describe('getQwenOAuthClient', () => {
    ...
    it('should handle cached credentials refresh failure', async () => {
        ...
        await expect(import('./qwenOAuth2.js').then((module) => module.getQwenOAuthClient(mockConfig))).rejects.toThrow('Cached Qwen credentials are invalid');
    });
});
...
describe('getQwenOAuthClient - Enhanced Error Scenarios', () => {
    ...
    it('should handle generic refresh token errors', async () => {
        ...
        vi.mocked(global.fetch).mockResolvedValue(mockRefreshResponse);
        await expect(import('./qwenOAuth2.js').then((module) => module.getQwenOAuthClient(mockConfig))).rejects.toThrow('Qwen token refresh failed: Token refresh failed: 500 Internal Server Error');
    });
    it('should handle different authentication failure reasons - timeout', async () => {
        ...
        await expect(import('./qwenOAuth2.js').then((module) => module.getQwenOAuthClient(mockConfig))).rejects.toThrow('Qwen OAuth authentication timed out');
    });
    it('should handle authentication failure reason - rate limit', async () => {
        ...
        await expect(import('./qwenOAuth2.js').then((module) => module.getQwenOAuthClient(mockConfig))).rejects.toThrow('Too many request for Qwen OAuth authentication, please try again later.');
    });
    it('should handle authentication failure reason - error', async () => {
        ...
        vi.mocked(global.fetch).mockResolvedValue(mockAuthResponse);
        await expect(import('./qwenOAuth2.js').then((module) => module.getQwenOAuthClient(mockConfig))).rejects.toThrow('Qwen OAuth authentication failed');
    });
});
