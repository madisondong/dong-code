...
describe('MemoryTool', () => {
    ...
    describe('performAddMemoryEntry (static method)', () => {
        let testFilePath;
        beforeEach(() => {
            testFilePath = path.join(os.homedir(), '.dong', DEFAULT_CONTEXT_FILENAME);
        });
    ...
    describe('execute (instance method)', () => {
        ...
        it('should call performAddMemoryEntry with correct parameters and return success', async () => {
            ...
            const expectedFilePath = path.join(os.homedir(), '.dong', getCurrentGeminiMdFilename());
    ...
    describe('shouldConfirmExecute', () => {
        ...
        it('should return confirmation details when memory file is not allowlisted', async () => {
            ...
            const result = await memoryTool.shouldConfirmExecute(params, mockAbortSignal);
            expect(result).toBeDefined();
            expect(result).not.toBe(false);
            if (result && result.type === 'edit') {
                const expectedPath = path.join('~', '.dong', 'DONG.md');
                expect(result.title).toBe(`Confirm Memory Save: ${expectedPath}`);
                expect(result.fileName).toContain(path.join('mock', 'home', '.dong'));
                expect(result.fileName).toContain('DONG.md');
                expect(result.fileDiff).toContain('Index: DONG.md');
                expect(result.fileDiff).toContain('+## Dong Added Memories');
                expect(result.fileDiff).toContain('+- Test fact');
                expect(result.originalContent).toBe('');
                expect(result.newContent).toContain('## Dong Added Memories');
                expect(result.newContent).toContain('- Test fact');
            }
        });
        it('should return false when memory file is already allowlisted', async () => {
            const params = { fact: 'Test fact' };
            const memoryFilePath = path.join(os.homedir(), '.dong', getCurrentGeminiMdFilename());
            // Add the memory file to the allowlist
            MemoryTool.allowlist.add(memoryFilePath);
            const result = await memoryTool.shouldConfirmExecute(params, mockAbortSignal);
        ...
        it('should add memory file to allowlist when ProceedAlways is confirmed', async () => {
            ...
            if (result && result.type === 'edit') {
                ...
                expect(MemoryTool.allowlist.has(memoryFilePath)).toBe(true);
            }
        });
        it('should not add memory file to allowlist when other outcomes are confirmed', async () => {
            ...
            if (result && result.type === 'edit') {
                ...
                expect(MemoryTool.allowlist.has(memoryFilePath)).toBe(false);
