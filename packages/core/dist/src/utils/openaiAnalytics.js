...
export class OpenAIAnalytics {
     ...
     */
    static async calculateStats(days = 7) {
        ...
        for (const logFile of logs) {
            try {
                ...
                if (!isObjectWith(logData, ['timestamp'])) {
                ...
                else if (isObjectWith(logData, ['error']) &&
                    logData.error) {
                    // Categorize errors
    ...
    static async generateReport(days = 7) {
        ...
        report += `### Models Used\n`;
        const sortedModels = Object.entries(stats.requestsByModel);
        ...
        if (Object.keys(stats.errorRates).length > 0) {
            report += `\n### Error Types\n`;
            const sortedErrors = Object.entries(stats.errorRates);
        ...
        for (let i = 0; i < 24; i++) {
            ...
            const barLength = maxRequests > 0 ? Math.round((requests / maxRequests) * scale) : 0;
     ...
     */
    static async saveReport(days = 7, outputPath) {
        const report = await this.generateReport(days);
        const reportPath = outputPath || path.join(process.cwd(), 'logs', 'openai', 'analytics.md');
        await fs.writeFile(reportPath, report, 'utf-8');
        return reportPath;
    }
}
function isObjectWith(obj, keys) {
    return (typeof obj === 'object' && obj !== null && keys.every((key) => key in obj));
}
/**
 * Extract the model name from a log entry
 */
function getModelFromLog(logData) {
    if (isObjectWith(logData, ['request', 'response'])) {
        const data = logData;
 ...
 */
function getTokenUsageFromLog(logData) {
    if (isObjectWith(logData, ['response'])) {
        const data = logData;
 ...
 */
function getErrorTypeFromLog(logData) {
    if (isObjectWith(logData, ['error'])) {
        const data = logData;
        if (data.error) {
            ...
            if (errorMsg.includes('content filter'))
...
//# sourceMappingURL=openaiAnalytics.js.map