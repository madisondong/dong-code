...
export class OpenAILogViewer {
    ...
    static async listLogs(limit) {
        try {
            ...
            for (let i = 0; i < logs.length; i++) {
                ...
                console.log(`${i + 1}. ${filename} - ${requestType} - ${status} - ${data.timestamp}`);
            }
        }
        catch (error) {
     ...
     */
    static async viewLog(identifier) {
        try {
            let logFile;
            ...
            if (typeof identifier === 'number') {
                ...
                if (identifier < 1 || identifier > logs.length) {
                    console.error(`Invalid log index. Please provide a number between 1 and ${logs.length}`);
                    return;
                }
     ...
     */
    static async cleanupLogs(keepCount = 50) {
        try {
            ...
            console.log(`Deleted ${logsToDelete.length} old log files. Kept ${keepCount} most recent logs.`);
        }
        catch (error) {
    ...
    async function main() {
        ...
        switch (command) {
            ...
            case 'view': {
                ...
                await OpenAILogViewer.viewLog(isNaN(Number(identifier)) ? identifier : Number(identifier));
                break;
            }
            case 'cleanup': {
            ...
            default:
                ...
                console.log('  list [limit]        - List all logs, optionally limiting to the specified number');
                console.log('  view <index|file>   - View a specific log by index number or filename');
                console.log('  cleanup [keepCount] - Remove old logs, keeping only the specified number (default: 50)');
                break;
        }
    }
...
//# sourceMappingURL=openaiLogViewer.js.map