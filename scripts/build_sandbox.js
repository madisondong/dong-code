...
console.log('packing @dong-code/dong-code ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'qwen-code-*.tgz'), { force: true });
execSync(
  `npm pack -w @dong-code/dong-code --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @dong-code/dong-code-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'qwen-code-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @dong-code/dong-code-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `dong-code-dong-code-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(
    corePackageDir,
    'dist',
    `dong-code-dong-code-core-${packageVersion}.tgz`,
  ),
  0o755,
);
